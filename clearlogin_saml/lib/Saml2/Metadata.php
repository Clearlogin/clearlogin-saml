...
/**
 * Metadata lib of Clearlogin PHP Toolkit
 *
 */

class Clearlogin_Saml2_Metadata
{
...
METADATA_TEMPLATE;
    ...
    {
        return Clearlogin_Saml2_Utils::addSign($metadata, $key, $cert);
    }
    ...
    public static function addX509KeyDescriptors($metadata, $cert)
    {
        ...
        try {
            $xml = Clearlogin_Saml2_Utils::loadXML($xml, $metadata);
            if (!$xml) {
        ...
        $formatedCert = Clearlogin_Saml2_Utils::formatCert($cert, false);
        $x509Certificate = $xml->createElementNS(Clearlogin_Saml2_Constants::NS_DS, 'X509Certificate', $formatedCert);

        $keyData = $xml->createElementNS(Clearlogin_Saml2_Constants::NS_DS, 'ds:X509Data');
        $keyData->appendChild($x509Certificate);

        $keyInfo = $xml->createElementNS(Clearlogin_Saml2_Constants::NS_DS, 'ds:KeyInfo');
        $keyInfo->appendChild($keyData);
        
        $keyDescriptor = $xml->createElementNS(Clearlogin_Saml2_Constants::NS_MD, "md:KeyDescriptor");
