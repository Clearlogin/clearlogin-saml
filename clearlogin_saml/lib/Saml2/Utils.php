...
/**
 * Utils of Clearlogin PHP Toolkit
 *
 * Defines several often used methods
 */

class Clearlogin_Saml2_Utils
{
    /**
    ...
    */
     ...
     * @param boolean $heads Clearlogin if we want to include head and footer
     *
     ...
     * @param boolean $stay       Clearlogin if we want to stay (returns the url string) False to redirect
     *
     * @return string $url
     */
    public static function redirect($url, $parameters = array(), $stay = false)
    {
        ...
        if (!preg_match('@^https?://@i', $url)) {
            throw new Clearlogin_Saml2_Error(
                ...
                Clearlogin_Saml2_Error::REDIRECT_INVALID_URL
            );
        }
    ...
    {
        return 'CLEARLOGIN_' . sha1(uniqid(mt_rand(), true));
    }
    ...
    public static function deleteLocalSession()
    {
        ...
        if (Clearlogin_Saml2_Utils::isSessionStarted()) {
            session_destroy();
        }
    ...
    */
    ...
    public static function validateSign ($xml, $cert = null, $fingerprint = null)
    {
        ...
        if (!empty($cert)) {
            ...
            $domCertFingerprint = Clearlogin_Saml2_Utils::calculateX509Fingerprint($domCert);
            if (Clearlogin_Saml2_Utils::formatFingerPrint($fingerprint) !== $domCertFingerprint) {
                return false;
            } else {
