 ...
 *
 */
class Clearlogin_Saml2_LogoutRequest
{
    /**
    ...
    */
     ...
     *
     */
    public function __construct(Clearlogin_Saml2_Settings $settings, $request = null, $nameId = null,$sessionIndex = null)
    {
        ...
        if (!isset($request) || empty($request)) {
            ...
            $id = Clearlogin_Saml2_Utils::generateUniqueID();
            $nameIdValue = Clearlogin_Saml2_Utils::generateUniqueID();
            $issueInstant = Clearlogin_Saml2_Utils::parseTime2SAML(time());
            ...
            if (!empty($nameId)) {
                ...
                $nameIdFormat = Clearlogin_Saml2_Constants::NAMEID_ENTITY;
            }

            $nameIdObj = Clearlogin_Saml2_Utils::generateNameId(
                $nameId,
                $spData['entityId'],
                $nameIdFormat,
                $cert
...
LOGOUTREQUEST;
    ...
    public static function getID($request)
    {
        if ($request instanceof DOMDocument) {
            ...
            $dom = new DOMDocument();
            $dom = Clearlogin_Saml2_Utils::loadXML($dom, $request);
        }
    ...
    public static function getNameIdData($request, $key = null)
    {
        ...
        $encryptedEntries = Clearlogin_Saml2_Utils::query($dom, '/samlp:LogoutRequest/saml:EncryptedID');
        ...
        if ($encryptedEntries->length == 1) {
            ...
            $entries = Clearlogin_Saml2_Utils::query($dom, '/samlp:LogoutRequest/saml:NameID');
            if ($entries->length == 1) {
    ...
    public static function getIssuer($request)
    {
        ...
        $issuer = null;
        $issuerNodes = Clearlogin_Saml2_Utils::query($dom, '/samlp:LogoutRequest/saml:Issuer');
    ...
    public static function getSessionIndexes($request)
    {
        ...
        $sessionIndexNodes = Clearlogin_Saml2_Utils::query($dom, '/samlp:LogoutRequest/samlp:SessionIndex');
    ...
    public function isValid($retrieveParametersFromServer=false)
    {
        ...
        try {
            $dom = new DOMDocument();
            $dom = Clearlogin_Saml2_Utils::loadXML($dom, $this->_logoutRequest);
            ...
            if ($this->_settings->isStrict()) {
                ...
                $res = Clearlogin_Saml2_Utils::validateXML($dom, 'saml-schema-protocol-2.0.xsd', $this->_settings->isDebugActive());
                ...
                if ($dom->documentElement->hasAttribute('NotOnOrAfter')) {
                    $na = Clearlogin_Saml2_Utils::parseSAML2Time($dom->documentElement->getAttribute('NotOnOrAfter'));
                    if ($na <= time()) {
            ...
            if (isset($_GET['Signature'])) {
                ...
                if ($retrieveParametersFromServer) {
                    $signedQuery = 'SAMLRequest='.Clearlogin_Saml2_Utils::extractOriginalQueryParam('SAMLRequest');
                    if (isset($_GET['RelayState'])) {
                        $signedQuery .= '&RelayState='.Clearlogin_Saml2_Utils::extractOriginalQueryParam('RelayState');
                    }
                    $signedQuery .= '&SigAlg='.Clearlogin_Saml2_Utils::extractOriginalQueryParam('SigAlg');
                } else {
                ...
                if ($signAlg != XMLSecurityKey::RSA_SHA1) {
                    try {
                        $objKey = Clearlogin_Saml2_Utils::castKey($objKey, $signAlg, 'public');
                    } catch (Exception $e) {
