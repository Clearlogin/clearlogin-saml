...
class Clearlogin_Saml2_Response
{

    /**
     * Settings
     * @var Clearlogin_Saml2_Settings
     */
    protected $_settings;
    ...
    */
     ...
     *
     * @param Clearlogin_Saml2_Settings $settings Settings.
     * @param string                  $response A UUEncoded SAML response from the IdP.
     */
    public function __construct(Clearlogin_Saml2_Settings $settings, $response)
    {
        ...
        $this->document = Clearlogin_Saml2_Utils::loadXML($this->document, $this->response);
        if (!$this->document) {
    ...
    */
    public function isValid($requestId = null)
    {
        ...
        try {
            ...
            if ($this->_settings->isStrict()) {
                ...
                $res = Clearlogin_Saml2_Utils::validateXML($this->document, 'saml-schema-protocol-2.0.xsd', $this->_settings->isDebugActive());
                ...
                $currentURL = Clearlogin_Saml2_Utils::getSelfRoutedURLNoQuery();
                ...
                if ($this->document->documentElement->hasAttribute('Destination')) {
                    ...
                    if (!empty($destination)) {
                        if (strpos($destination, $currentURL) !== 0) {
                            $currentURLrouted = Clearlogin_Saml2_Utils::getSelfRoutedURLNoQuery();
                ...
                foreach ($subjectConfirmationNodes as $scn) {
                    if ($scn->hasAttribute('Method') && $scn->getAttribute('Method') != Clearlogin_Saml2_Constants::CM_BEARER) {
                        continue;
                    }
                    ...
                    if ($subjectConfirmationDataNodes->length == 0) {
                        ...
                        if ($scnData->hasAttribute('NotOnOrAfter')) {
                            $noa = Clearlogin_Saml2_Utils::parseSAML2Time($scnData->getAttribute('NotOnOrAfter'));
                            if ($noa <= time()) {
                                continue;
                            }
                        }
                        if ($scnData->hasAttribute('NotBefore')) {
                            $nb = Clearlogin_Saml2_Utils::parseSAML2Time($scnData->getAttribute('NotBefore'));
                            if ($nb > time()) {
                                continue;
                            }
                        }
            ...
            if (!empty($signedElements)) {
                ...
                if (in_array('Response', $signedElements)) {
                    ...
                    if ($this->encrypted) {
                        $encryptedIDNodes = Clearlogin_Saml2_Utils::query($this->decryptedDocument, '/samlp:Response/saml:EncryptedAssertion/saml:Assertion/saml:Subject/saml:EncryptedID');
                        if ($encryptedIDNodes->length > 0) {
                ...
                if (!Clearlogin_Saml2_Utils::validateSign($documentToValidate, $cert, $fingerprint)) {
    ...
    {
        $status = Clearlogin_Saml2_Utils::getStatus($this->document);

        if (isset($status['code']) && $status['code'] !== Clearlogin_Saml2_Constants::STATUS_SUCCESS) {
    ...
    public function getNameIdData()
    {
        ...
        if ($encryptedIdDataEntries->length == 1) {
            ...
            $nameId = Clearlogin_Saml2_Utils::decryptElement($encryptedData, $seckey);
    ...
    public function getSessionNotOnOrAfter()
    {
        ...
        if ($entries->length !== 0) {
            $notOnOrAfter = Clearlogin_Saml2_Utils::parseSAML2Time($entries->item(0)->getAttribute('SessionNotOnOrAfter'));
        }
        return $notOnOrAfter;
    }
    ...
    public function getAttributes()
    {
        ...
        if ($encriptedAttributes->length > 0) {
            foreach ($encriptedAttributes as $encriptedAttribute) {
                ...
                $seckey = new XMLSecurityKey(XMLSecurityKey::RSA_1_5, array('type'=>'private'));
                $seckey->loadKey($key);
                $attribute = Clearlogin_Saml2_Utils::decryptElement($encriptedAttribute->firstChild(), $seckey);
            }
        }
        */
    ...
    public function validateTimestamps()
    {
        ...
        for ($i = 0; $i < $timestampNodes->length; $i++) {
            ...
            if ($nbAttribute && Clearlogin_SAML2_Utils::parseSAML2Time($nbAttribute->textContent) > time() + Clearlogin_Saml2_Constants::ALOWED_CLOCK_DRIFT) {
                return false;
            }
            if ($naAttribute && Clearlogin_SAML2_Utils::parseSAML2Time($naAttribute->textContent) + Clearlogin_Saml2_Constants::ALOWED_CLOCK_DRIFT <= time()) {
                return false;
            }
        }
        return true;
    ...
    private function _queryAssertion($assertionXpath)
    {
        ...
        $xpath->registerNamespace('samlp', Clearlogin_Saml2_Constants::NS_SAMLP);
        $xpath->registerNamespace('saml', Clearlogin_Saml2_Constants::NS_SAML);
        $xpath->registerNamespace('ds', Clearlogin_Saml2_Constants::NS_DS);
        $xpath->registerNamespace('xenc', Clearlogin_Saml2_Constants::NS_XENC);
    ...
    {
        return Clearlogin_Saml2_Utils::query($this->document, $query);
    }
