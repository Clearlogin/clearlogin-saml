...
/**
 * Configuration of the Clearlogin PHP Toolkit
 *
 */

class Clearlogin_Saml2_Settings
{
    /**
     * List of paths.
     *
    ...
    public function __construct($settings = null)
    {
        ...
        if (!isset($settings)) {
            if (!$this->_loadSettingsFromFile()) {
                throw new Clearlogin_Saml2_Error(
                    'Invalid file settings: %s',
                    Clearlogin_Saml2_Error::SETTINGS_INVALID,
    ...
    private function _loadPaths()
    {
        ...
        if (defined('CLEARLOGIN_CUSTOMPATH')) {
            $this->_paths['config'] = CLEARLOGIN_CUSTOMPATH;
            $this->_paths['cert'] = CLEARLOGIN_CUSTOMPATH.'certs/';
        }
    }
     ...
     * 
     * @return boolean  Clearlogin if the settings info is valid
     */
    ...
    private function _loadSettingsFromFile()
    {
        ...
        if (!file_exists($filename)) {
            throw new Clearlogin_Saml2_Error(
                ...
                Clearlogin_Saml2_Error::SETTINGS_FILE_NOT_FOUND,
                array($filename)
            );
        }
    ...
    private function _addDefaultValues()
    {
        if (!isset($this->_sp['assertionConsumerService']['binding'])) {
            $this->_sp['assertionConsumerService']['binding'] = Clearlogin_Saml2_Constants::BINDING_HTTP_POST;
        }
        if (isset($this->_sp['singleLogoutService']) && !isset($this->_sp['singleLogoutService']['binding'])) {
            $this->_sp['singleLogoutService']['binding'] = Clearlogin_Saml2_Constants::BINDING_HTTP_REDIRECT;
        }

        // Related to nameID
        if (!isset($this->_sp['NameIDFormat'])) {
            $this->_sp['NameIDFormat'] = Clearlogin_Saml2_Constants::NAMEID_PERSISTENT;
        }
    ...
    {
        $metadata = Clearlogin_Saml2_Metadata::builder($this->_sp, $this->_security['authnRequestsSigned'], $this->_security['wantAssertionsSigned'], null, null, $this->getContacts(), $this->getOrganization());

        $cert = $this->getSPcert();

        if (!empty($cert)) {
            $metadata = Clearlogin_Saml2_Metadata::addX509KeyDescriptors($metadata, $cert);
        }
        ...
        if (isset($this->_security['signMetadata']) && $this->_security['signMetadata'] !== false) {
            if ($this->_security['signMetadata'] === true) {
                ...
                if (!$keyMetadata) {
                    throw new Clearlogin_Saml2_Error(
                        ...
                        Clearlogin_Saml2_Error::PRIVATE_KEY_FILE_NOT_FOUND
                    );
                }
                ...
                if (!$certMetadata) {
                    throw new Clearlogin_Saml2_Error(
                        ...
                        Clearlogin_Saml2_Error::PUBLIC_CERT_FILE_NOT_FOUND
                    );
                }
            } else {
                if (!isset($this->_security['signMetadata']['keyFileName'])
                    ...
                    throw new Clearlogin_Saml2_Error(
                        ...
                        Clearlogin_Saml2_Error::SETTINGS_INVALID_SYNTAX
                    );
                }
            ...
            $metadata = Clearlogin_Saml2_Metadata::signMetadata($metadata, $keyMetadata, $certMetadata);
        }
        return $metadata;
    }
    ...
    public function validateMetadata($xml)
    {
        ...
        $errors = array();
        $res = Clearlogin_Saml2_Utils::validateXML($xml, 'saml-schema-metadata-2.0.xsd', $this->_debug);
        if (!$res instanceof DOMDocument) {
            ...
            if ($element->tagName !== 'md:EntityDescriptor') {
                ...
                if ($element->hasAttribute('validUntil')) {
                    $validUntil = Clearlogin_Saml2_Utils::parseSAML2Time($element->getAttribute('validUntil'));
                }
                if ($element->hasAttribute('cacheDuration')) {
                    $cacheDuration = $element->getAttribute('cacheDuration');
                }

                $expireTime = Clearlogin_Saml2_Utils::getExpireTime($cacheDuration, $validUntil);
    ...
    {
        if (isset($this->_idp['x509cert'])) {
            $this->_idp['x509cert'] = Clearlogin_Saml2_Utils::formatCert($this->_idp['x509cert']);
        }
    }
    ...
    {
        if (isset($this->_sp['x509cert'])) {
            $this->_sp['x509cert'] = Clearlogin_Saml2_Utils::formatCert($this->_sp['x509cert']);
        }
    }
    ...
    {
        if (isset($this->_sp['privateKey'])) {
            $this->_sp['privateKey'] = Clearlogin_Saml2_Utils::formatPrivateKey($this->_sp['privateKey']);
        }
    }
