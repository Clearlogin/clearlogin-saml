 ...
 *
 * SAML is very powerful and flexible, but the specification can be quite a handful. Now Clearlogon is
 ...
 */
	...
	/**
	 * Clearlogin SAML for Moodle - base definition
	**/
	class auth_plugin_clearlogin_saml extends auth_plugin_base {

		/**
		* Constructor.
		*/
		function auth_plugin_clearlogin_saml() {
			$this->authtype = 'clearlogin_saml';
			$this->config = get_config('auth/clearlogin_saml');
		}

		/**
		* Returns clearlogin if the username and password work and false if they are
		...
		*/
		function user_login($username, $password) {
			// if clearlogin, user_login was initiated by clearlogin_saml/index.php
			if (isset($GLOBALS['clearlogin_saml_login_attributes'])) {
				return TRUE;
			}
			return FALSE;
		}
		...
		*/
		function get_userinfo($username=null) {
			...
			$saml_attributes = $GLOBALS['clearlogin_saml_login_attributes'];
			$nameID = $GLOBALS['clearlogin_saml_nameID'];
		...
		*/
		...
		/**
		* Returns clearlogin if this authentication plugin is 'internal'.
		*
		* @return bool
		*/
		...
		/**
		* Returns clearlogin if this authentication plugin can change the user's
		* password.
		*
		* @return bool
		*/
		...
		function loginpage_hook() {
			...
			if (!isset($_GET['normal']) && (empty($_POST['username']) && empty($_POST['password']))) {
				$init_sso_url = $GLOBALS['CFG']->wwwroot.'/auth/clearlogin_saml/index.php';
		...
		function logoutpage_hook() {
			...
			$logout_url = $GLOBALS['CFG']->wwwroot.'/auth/clearlogin_saml/index.php?logout=1';
		...
		*/
		...
		function validate_form($form, &$err) {
			if (empty($form->idp_sso_issuer_url)) {
				$err['idp_sso_issuer_url_empty'] = '"'.get_string('auth_clearlogin_saml_idp_sso_issuer_url', 'auth_clearlogin_saml').'" '. get_string('auth_clearlogin_saml_empty_required_value', 'auth_clearlogin_saml');
			}
			if (empty($form->idp_sso_target_url)) {
				$err['idp_sso_target_url_empty'] = '"'.get_string('auth_clearlogin_saml_idp_sso_target_url', 'auth_clearlogin_saml').'" '. get_string('auth_clearlogin_saml_empty_required_value', 'auth_clearlogin_saml');
			}

			if (!empty($form->saml_auto_create_users) || !empty($form->saml_auto_update_users)) {

				if (empty($form->saml_username_map)) {
					$err['saml_username_map_empty'] = get_string('auth_clearlogin_saml_create_or_update_warning', 'auth_clearlogin_saml').' "'.get_string('auth_clearlogin_saml_username_map', 'auth_clearlogin_saml').'" '. get_string('auth_clearlogin_saml_empty_required_value', 'auth_clearlogin_saml');
				}
				if (empty($form->saml_email_map)) {
					$err['saml_email_map_empty'] = get_string('auth_clearlogin_saml_create_or_update_warning', 'auth_clearlogin_saml').' "'.get_string('auth_clearlogin_saml_email_map', 'auth_clearlogin_saml').'" '. get_string('auth_clearlogin_saml_empty_required_value', 'auth_clearlogin_saml');
				}
				if (empty($form->saml_firstname_map)) {
					$err['saml_firstname_map_empty'] = get_string('auth_clearlogin_saml_create_or_update_warning', 'auth_clearlogin_saml').' "'.get_string('auth_clearlogin_saml_firstname_map', 'auth_clearlogin_saml').'" '. get_string('auth_clearlogin_saml_empty_required_value', 'auth_clearlogin_saml');
				}
				if (empty($form->saml_surname_map)) {
					$err['saml_surname_map_empty'] = get_string('auth_clearlogin_saml_create_or_update_warning', 'auth_clearlogin_saml').' "'.get_string('auth_clearlogin_saml_surname_map', 'auth_clearlogin_saml').'" '. get_string('auth_clearlogin_saml_empty_required_value', 'auth_clearlogin_saml');
				}
			}
		}
		...
		*/
		function process_config($config) {
			...
			set_config('idp_sso_target_url', trim($config->idp_sso_target_url), 'auth/clearlogin_saml');
			set_config('idp_sso_issuer_url', trim($config->idp_sso_issuer_url), 'auth/clearlogin_saml');
			set_config('idp_slo_target_url', trim($config->idp_slo_target_url), 'auth/clearlogin_saml');
			set_config('x509certificate', trim($config->x509certificate), 'auth/clearlogin_saml');
			set_config('saml_auto_create_users',  $config->saml_auto_create_users, 'auth/clearlogin_saml');

			set_config('saml_auto_update_users',  $config->saml_auto_update_users, 'auth/clearlogin_saml');
			set_config('saml_slo',  $config->saml_slo, 'auth/clearlogin_saml');
			set_config('saml_account_matcher',  $config->saml_account_matcher, 'auth/clearlogin_saml');
			set_config('saml_username_map',  trim($config->saml_username_map), 'auth/clearlogin_saml');
			set_config('saml_email_map',  trim($config->saml_email_map), 'auth/clearlogin_saml');
			set_config('saml_email_map',  trim($config->saml_email_map), 'auth/clearlogin_saml');
			set_config('saml_firstname_map',  trim($config->saml_firstname_map), 'auth/clearlogin_saml');
			set_config('saml_surname_map',  trim($config->saml_surname_map), 'auth/clearlogin_saml');
			set_config('saml_role_map',  trim($config->saml_role_map), 'auth/clearlogin_saml');
			set_config('saml_role_siteadmin_map',  trim($config->saml_role_siteadmin_map), 'auth/clearlogin_saml');
			set_config('saml_role_coursecreator_map',  trim($config->saml_role_coursecreator_map), 'auth/clearlogin_saml');
			set_config('saml_role_manager_map',  trim($config->saml_role_manager_map), 'auth/clearlogin_saml');
			set_config('saml_debug_mode',  $config->saml_debug_mode, 'auth/clearlogin_saml');
			set_config('saml_strict_mode',  $config->saml_strict_mode, 'auth/clearlogin_saml');
			set_config('sp_entity_id',  trim($config->sp_entity_id), 'auth/clearlogin_saml');
			set_config('saml_nameid_encrypted',  $config->saml_nameid_encrypted, 'auth/clearlogin_saml');
			set_config('saml_authn_request_signed',  $config->saml_authn_request_signed, 'auth/clearlogin_saml');
			set_config('saml_logout_request_signed',  $config->saml_logout_request_signed, 'auth/clearlogin_saml');
			set_config('saml_logout_response_signed',  $config->saml_logout_response_signed, 'auth/clearlogin_saml');
			set_config('saml_want_message_signed',  $config->saml_want_message_signed, 'auth/clearlogin_saml');
			set_config('saml_want_assertion_signed',  $config->saml_want_assertion_signed, 'auth/clearlogin_saml');
			set_config('saml_want_assertion_encrypted',  $config->saml_want_assertion_encrypted, 'auth/clearlogin_saml');
			set_config('sp_x509cert',  trim($config->sp_x509cert), 'auth/clearlogin_saml');
			set_config('sp_privatekey',  trim($config->sp_privatekey), 'auth/clearlogin_saml');
		...
		public function test_settings() {
			...
			$pluginconfig = get_config('auth/clearlogin_saml');
			...
			try {
				$samlSettings = new Clearlogin_Saml2_Settings($settings);
			...
			$fileSystemKeyExists = file_exists($CFG->dirroot.'/auth/clearlogin_saml/certs/sp.key');
			$fileSystemCertExists = file_exists($CFG->dirroot.'/auth/clearlogin_saml/certs/sp.crt');
			if ($fileSystemKeyExists) {
				$privatekey_url = $CFG->wwwroot . '/auth/clearlogin_saml/certs/sp.key';
			...
			$attr_mappings = array (
				'saml_username_map' => get_string("auth_clearlogin_saml_username_map", "auth_clearlogin_saml"),
				'saml_email_map' => get_string("auth_clearlogin_saml_email_map", "auth_clearlogin_saml"),
				'saml_firstname_map' => get_string("auth_clearlogin_saml_firstname_map", "auth_clearlogin_saml"),
				'saml_surname_map' => get_string("auth_clearlogin_saml_surname_map", "auth_clearlogin_saml"),
				'saml_role_map' => get_string("auth_clearlogin_saml_role_map", "auth_clearlogin_saml"),
			);
			...
			$role_mappings = array (
				'saml_role_siteadmin_map' => get_string("auth_clearlogin_saml_rolemapping_head", "auth_clearlogin_saml"),
				'saml_role_coursecreator_map' => get_string("auth_clearlogin_saml_role_coursecreator_map", "auth_clearlogin_saml"),
				'saml_role_manager_map' => get_string("auth_clearlogin_saml_role_manager_map", "auth_clearlogin_saml"),
			);
