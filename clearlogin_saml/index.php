 ...
 *
 * index.php - landing page for auth/clearlogin_saml
 *
 ...
 */
	...
	/**
	 * check that the saml session is OK - if not, send to Clearlogin for authentication
	...
	if ($retry == $SAML_RETRIES) {
		...
		session_write_close();
		print_error('retriesexceeded', 'auth_clearlogin_saml', '', $retry);
	}
	...
	if (!isset($_POST['SAMLResponse']) && !((isset($_GET['normal']) && $_GET['normal']) || (isset($SESSION->normal) && $SESSION->normal))  && !(isset($_GET['logout']) && $_GET['logout'])) {
		...
		try {
			...
			$errors = $auth->getErrors();
			if (empty($errors)) {
				$GLOBALS['clearlogin_saml_nameID'] = $clearlogin_saml_nameId = $auth->getNameId();
				$GLOBALS['clearlogin_saml_login_attributes'] = $saml_attributes = $auth->getAttributes();
	...
	$GLOBALS['clearlogin_saml_login'] = TRUE;

	$samlplugin = get_auth_plugin('clearlogin_saml');
	$saml_user = $samlplugin->get_userinfo(null);

	// check user name attribute actually passed
	if($saml_user == false){
		error_log('auth_clearlogin_saml: auth failed due to missing username/email saml attribute: '.$pluginconfig->saml_username_map);
		session_write_close();
		$USER = new object();
		$USER->id = 0;
		require_once('../../config.php');
		print_error('auth_clearlogin_saml: auth failed due to missing username/email saml attribute: '.$pluginconfig->saml_username_map."<br />".get_string("auth_clearlogin_saml_username_email_error", "auth_clearlogin_saml")."\r\n");
	}
	...
	if ($_POST['SAMLResponse']) {
		...
		$USER = auth_clearlogin_saml_authenticate_user_login($saml_account_matcher, $saml_user, $saml_create, $saml_update);
	} else {
	...
	if ($USER == false) {
		...
		$USER->id = 0;
		require_once('../../config.php');
		print_error('pluginauthfailed', 'auth_clearlogin_saml', '', (!empty($saml_user['username']) ? $saml_user['username'] : $saml_user['email']));
	}
