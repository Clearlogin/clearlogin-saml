 ...
 */
		...
		$settings = array (
		    ...
		    'sp' => array (
		        ...
		        'assertionConsumerService' => array (
		            'url' => htmlspecialchars($CFG->wwwroot.'/auth/clearlogin_saml/index.php'),
		        ),
		        'singleLogoutService' => array (
		            'url' => htmlspecialchars($CFG->wwwroot.'/auth/clearlogin_saml/index.php?logout=1'),
		        ),
	 ...
	 */
	function auth_clearlogin_saml_authenticate_user_login($saml_account_matcher, $user_saml, $saml_create=false, $saml_update=false) {
	...
	function logoutpage_hook() {
		global $CFG;
		redirect($CFG->wwwroot.'/auth/clearlogin_saml/index.php?logout=1', 0);
	}
	 ...
	 * (for config.php)
	 */
	function auth_clearlogin_saml_addsingleslashes($input){
	 ...
	 * @return string
	 */
	function auth_clearlogin_saml_qualified_me() {
	 ...
	 * @return string
	 */
	function auth_clearlogin_saml_me() {
	...
	function auth_clearlogin_saml_err($msg) {
		$stderr = fopen('php://stderr', 'w');
		fwrite($stderr,"auth_plugin_clearlogin_saml: ". $msg . "\n");
		fclose($stderr);
	}
